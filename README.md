...
![image](https://github.com/user-attachments/assets/26360490-b467-4f13-9a2d-eebd06498c52)
    ...
    ids_restore =argsort(ids_selected)  #recover the original order
    ids_selected = ids_selected[:, :(ids_selected.shape[1]*0.75)]  #select 25% 
    x = gather(x, ids_selected) #(b,0.25*H/p*W/p,C)
    mask = gather(mask, ids_restore)
    # 1.4 add cls token
    cls_tokens = zeros(1,1,C) + embed_pos[:,:1,:]
