...
for epoch in range(num_epochs):
    ...
    for batch in tqdm(dataloader, desc="train", leave=False):
        ...
        prefix_labels = torch.full(
            (input['input_ids'].shape[0], num_virtual_tokens),
            -100).to(labels.device)
        labels = torch.cat((prefix_labels, labels.long()), dim=1)
        mask = (labels > 0).bool()

        loss = torch.mean(criterior(logits.permute(0, 2, 1), labels)[mask])
    ...
    with torch.no_grad():
        ...
        test_outputs = model.generate(input_ids=test_inputs["input_ids"],
                                    attention_mask=test_inputs["attention_mask"],
                                    max_new_tokens=10,
                                    eos_token_id=tokenizer.eos_token_id)

        print(
            tokenizer.batch_decode(test_outputs.detach().cpu().numpy(),
