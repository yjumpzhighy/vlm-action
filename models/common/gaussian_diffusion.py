...
class GaussianDiffusion(nn.Module):
    def __init__(self, model, image_size, image_channel, timesteps, objective="pred_noise",
                 device=None):
        super().__init__()
        self.model = model
        ...
        self.self_condition = model.self_condition if hasattr(model, "self_condition") else None
        ...
        sqrt_reciprocal_minus_one_alphas_cumprod = torch.sqrt(1./alphas_cumprod-1)
        ...
        self.register_buffer('sqrt_alphas_cumprod', sqrt_alphas_cumprod.to(torch.float32))
        self.register_buffer('sqrt_one_minus_alphas_cumprod', sqrt_one_minus_alphas_cumprod.to(torch.float32))
        self.register_buffer('log_one_minus_alphas_cumprod', log_one_minus_alphas_cumprod.to(torch.float32))
        self.register_buffer('sqrt_reciprocal_alphas_cumprod', sqrt_reciprocal_alphas_cumprod.to(torch.float32))
        self.register_buffer('sqrt_reciprocal_minus_one_alphas_cumprod', sqrt_reciprocal_minus_one_alphas_cumprod.to(torch.float32))
        self.register_buffer('posterior_var', posterior_var.to(torch.float32))
        self.register_buffer('posterior_log_var_clipped', posterior_log_var_clipped.to(torch.float32))
    ...
    def sample(self, batch_size=1):
        img = torch.randn((batch_size,self.image_channel,self.image_size,self.image_size),
                          device=self.device)
    ...
    def p_sample(self, xt, t, x_self_cond):
        ...
        model_mean, model_logvar, x0 = self.p_mean_var(xt,batched_times,x_self_cond,clip_denoise=True)
...
if __name__ == "__main__":
    ...
    from unet import UnetCondition
    import torchvision
    ...
    model = UnetCondition(IMAGE_SIZE,IMAGE_SIZE,IMAGE_C,base_channel=64,output_channel=3)
